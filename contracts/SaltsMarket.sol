...
contract SaltsMarketPlace is Ownable {
    ...
    constructor(
        ...
        commision[3] = 1;
        commision[4] = c;
        initialLockedBUSD = busdAmount;
    }

    // initial locked busd for calculating MSP
    uint256 public initialLockedBUSD;
    ...
    function SELL(uint256 saltsAmount, address _referer) public {
        ...
        if(SALTS.balanceOf(msg.sender) == 0) {
    ...
    // returns Minimum Selling Price
    function MSP() public returns(uint256) {
        uint256 burnedTokens_in_BUSD = SALTS.BurnedTokens() * CMP_Salts_to_Busd();
        uint256 msp = (initialLockedBUSD + burnedTokens_in_BUSD) / SALTS.currentSupply();
        return msp;
    }
